import { FaBroadcastTower, FaPaperPlane, FaSatelliteDish } from "react-icons/fa";
...
function RoomChat({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") onSendChatMessage();
  };
      ...
      <div className="card-header">
        <div className="card-header-left">
          ...
          <div className="card-title">
            <FaBroadcastTower size={22} color="var(--blue)" />
            Broadcast
          </div>
        </div>
          ...
          chatEvents.map((event, i) => (
            <div
              className={`chat-bubble ${
                event.payload?.role === "teacher" ? "teacher-bubble" : ""
              }`}
              key={i}
            >
              <div className="chat-meta">
                ...
                <span
                  className={
                    event.payload?.role === "teacher" ? "teacher-role" : ""
                  }
                >
                  {event.payload?.role}
                </span>
